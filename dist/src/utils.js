...
export function removeQuotedText(text) {
...
export function sortThread(thread) {
    return thread.sort(({ date: dateA }, { date: dateB }) => {
        const a = dayjs(dateA);
        const b = dayjs(dateB);
        if (!dateA || !dateB || a.isSame(b))
            return 0;
...
export function sortThreadsByFirstMessage(threads) {
...
export function findThreadRoot(inReplyTo, messageMap, references) {
