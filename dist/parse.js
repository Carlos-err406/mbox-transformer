...
program
    ...
    .option("-m, --min-messages <number>", "minimum number of messages per thread, defaults to 0")
    .parse(process.argv);
const options = program.opts();
const inputFile = options.input;
const outputFolder = options.output;
const minMessages = (_a = Number(options.minMessages)) !== null && _a !== void 0 ? _a : 0;
...
async function main() {
    try {
        ...
        const sorted = sortThreadsByFirstMessage(threadArray.map(sortThread)).filter((thread) => thread.length >= minMessages);
